use std::{ptr::NonNull, marker::PhantomData, fmt::{Display, Formatter,Result}};


struct Node<T>{
    val:T,
    next:Option<NonNull<Node<T>>>,//NonNull是裸指针的封装，保证裸指针不为空（指向某个地址）且协变
...
impl<T> LinkedList<T>{
    ...
    fn insert_at_head(&mut self, obj: T) {
        ...
        node.next = self.head;
        node.prev = None;
        let node_ptr = Some(unsafe { NonNull::new_unchecked(Box::into_raw(node)) });//写入：这里将node装箱并将裸指针封装进NonNull
        match self.head {
            None => self.tail = node_ptr,
            Some(head_ptr) => unsafe { (*head_ptr.as_ptr()).prev = node_ptr },//读取：用*寻址
        }
        self.head = node_ptr;
        self.length += 1;
    }

    fn insert_at_tail(&mut self, obj: T) {
        let mut node = Box::new(Node::new(obj));
        node.next = None;
        node.prev = self.tail;
    ...
    fn insert_at_ith(&mut self, index: u32, obj: T) {
    ...
    fn delete_head(&mut self) -> Option<T> {
        ...
        self.head.map(|head_ptr| unsafe {
            ...
            match old_head.next {
                Some(mut next_ptr) => next_ptr.as_mut().prev = None,//as_mut返回的是Node<T>的唯一引用
                None => self.tail = None,
            }
    ...
    fn delete_tail(&mut self) -> Option<T> {
        ...
        self.tail.map(|tail_ptr| unsafe {
            let old_tail = Box::from_raw(tail_ptr.as_ptr());//从NonNull中取出值并装箱
            match old_tail.prev {
    ...
    fn get_ith_node(&mut self, node: Option<NonNull<Node<T>>>, index: i32) -> Option<&T> {
        match node {
            ...
            Some(next_ptr) => match index {
                0 => Some(unsafe { &(*next_ptr.as_ptr()).val }),//这里返回的是&T
